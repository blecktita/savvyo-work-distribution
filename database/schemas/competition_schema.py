...
class Competition(Base):
    ...
    competition_type = Column(
        String(100), nullable=True, doc="Type/category of the competition"
    )

    # Geographic and tier information
    country = Column(
        String(100), nullable=True, doc="Country where the competition is held"
    )
    tier = Column(
        String(100),
        nullable=True,
    ...
    number_of_clubs = Column(
        Integer, default=0, nullable=False, doc="Total number of clubs participating"
    )
    ...
    percentage_of_foreign_players: Column[int] = Column(
        Integer, default=0, nullable=False, doc="Percentage of foreign players (0-100)"
    )
    percentage_game_ratio_of_foreign_players: Column[float] = Column(
        Float, default=0.0, nullable=False, doc="Game time ratio for foreign players"
    )
    goals_per_match: Column[float] = Column(
        Float, default=0.0, nullable=False, doc="Average goals scored per match"
    )

    # Market value data
    average_market_value: Column[float] = Column(
        Float, default=0.0, nullable=False, doc="Average market value per player"
    )
    total_market_value: Column[float] = Column(
        Float, default=0.0, nullable=False, doc="Total market value of all players"
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "percentage_game_ratio_of_foreign_players": self.percentage_game_ratio_of_foreign_players,
