# database/match_models_updated.py
...
class LeagueTableEntry(Base):
    ...
    team = relationship('Team', back_populates='league_entries')
...
class Substitution(Base):
    ...
    id = Column(Integer, primary_key=True)  # Your DB uses INTEGER id, not substitution_id
    match_id = Column(String(50), ForeignKey('matches.match_id'))
    ...
    player_out = relationship('Player', back_populates='subs_out', foreign_keys=[player_out_id])
    player_in = relationship('Player', back_populates='subs_in', foreign_keys=[player_in_id])
...
class Goal(Base):
    ...
    assist_player = relationship('Player', back_populates='assisted_goals', foreign_keys=[assist_player_id])
...
class Player(Base):
    ...
    assisted_goals = relationship('Goal', back_populates='assist_player', foreign_keys='Goal.assist_player_id')
    cards = relationship('Card', back_populates='player')
    subs_out = relationship('Substitution', back_populates='player_out', foreign_keys='Substitution.player_out_id')
    subs_in = relationship('Substitution', back_populates='player_in', foreign_keys='Substitution.player_in_id')
...
class MatchdaySummary(Base):
    ...
    matchday_id = Column(String(50), ForeignKey('matchdays.matchday_id'), primary_key=True)
