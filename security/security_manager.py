...
class IPSecurityManager:
    ...
    def __init__(
        ...
        self.alert_system = AlertSystem(
            env_file_path=self.env_config.env_file_path
        )
    ...
    def check_request(self) -> None:
        ...
        if self.reboot_required:
            try:
                if self._attempt_full_recovery():
                    ...
                    raise IPSecurityViolationError("Full recovery failed - manual intervention required")
            except Exception as e:
        ...
        with self.rotation_lock:
            try:
                ...
                self._send_critical_alert(error_msg, self.current_ip, self.request_count)
                raise IPSecurityViolationError(error_msg)
    ...
    def _handle_rotation_failure(self, old_ip: str, reason: str, error: str) -> None:
        ...
        error_msg = f"IP rotation failed ({reason}): {error}. Manual intervention required."
        
        self._send_critical_alert(
            f"ROTATION FAILURE: {reason} - {error}",
            old_ip,
            self.request_count
        )
    ...
    def get_security_status(self) -> Dict:
        try:
            ...
            monitor_status = {
                ...
                "monitoring_active": getattr(self.rotation_monitor, 'is_monitoring', False),
                "should_rotate": False,
                "total_rotations": len(getattr(self.rotation_monitor, 'rotation_history', [])),
