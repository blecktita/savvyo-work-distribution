...
class SmartSeasonTerminator:
    ...
    def _analyze_with_sampling(
        ...
        """
        print(f"🔍 Pattern detected for {competition_id}: {self.consecutive_failures[competition_id]} consecutive failures")
        print(f"   Failed season: {failed_season['season_id']} ({failed_season['year']})")
        
        # Get samples to test
        sample_seasons = self._generate_sample_seasons(
            failed_season, remaining_seasons
        )
    ...
    def _make_termination_decision(
        ...
        if failure_rate >= self.config.stop_threshold:
            ...
            if successful_samples:
                ...
                cutoff_year = failed_season['year']
                reason = f"High failure rate ({failure_rate:.1%}), no data found in samples"
    ...
    def get_termination_summary(self, competition_id: str) -> Dict:
        ...
        return {
            ...
            'failure_rate': sum(1 for s in samples if not s.has_data) / len(samples) if samples else 0,
            'consecutive_failures': self.consecutive_failures.get(competition_id, 0),
            'samples_by_year': {s.year: {'has_data': s.has_data, 'clubs': s.clubs_found} for s in samples}
        }
...
def create_season_tester(club_orchestrator, driver):
    ...
    def test_season_has_data(competition_id: str, season: Dict[str, str]) -> tuple:
        ...
        try:
            ...
            season_url = club_orchestrator._construct_season_url(competition_url, season_year)
