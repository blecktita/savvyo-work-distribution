...
def analyze_sqlite_database(db_path: str):
    ...
    for sqlite_table, postgres_table in migration_tables.items():
        if sqlite_table in tables:
            ...
            will_migrate.append(f"✅ {sqlite_table} → {postgres_table} ({row_count:,} rows)")
        else:
...
def suggest_migration_modifications(table_info):
    ...
    extra_tables = []
    standard_tables = {'teams', 'competitions', 'players', 'matches', 'goals', 'cards', 'substitutions'}
    
    for table_name in table_info.keys():
        if table_name not in standard_tables and table_info[table_name]['row_count'] > 0:
...
def main():
    ...
    db_path = input("Enter path to your football.db file (or press Enter for 'football.db'): ").strip()
    if not db_path:
        db_path = "football.db"
