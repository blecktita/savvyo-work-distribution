...
class ScraperConfig:
    ...
    database: DatabaseConfig = field(default_factory=lambda: DatabaseConfig.development())
    ...
    def validate(self) -> bool:
        ...
        if self.use_vpn:
            ...
            if self.vpn.request_max_delay < self.vpn.request_min_delay:
                raise ValueError("vpn_request_max_delay must be >= vpn_request_min_delay")
    ...
    def get_summary(self) -> dict:
        ...
        summary = {
            ...
            'vpn': {
                'enabled': self.use_vpn,
                'avg_delay': avg_delay
            },
            'database': {
                'save_enabled': self.save_to_database,
                'url_type': (self.database.database_url.split('://')[0] 
                           if '://' in self.database.database_url else 'unknown'),
    ...
    def __post_init__(self):
        ...
        try:
            ...
            print(f"Warning: Could not create output directory {self.output_directory}: {e}")
