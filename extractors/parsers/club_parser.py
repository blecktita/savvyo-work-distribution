...
from extractors.extractor_club import ClubDataExtractor, ClubRowExtractor
...
class ClubTableParser(BaseParser):
    ...
    def _process_all_club_rows(
        self, rows: List[Tag], season_year: str, season_id: str, competition_id: str
    ) -> List[Dict]:
        """
    ...
    def _should_skip_club_row(self, row: Tag) -> bool:
        ...
        """
        return self._should_skip_header_row(row) or self._should_skip_footer_row(row)

    def _process_club_row(
        self, row: Tag, season_year: str, season_id: str, competition_id: str
    ) -> Optional[Dict]:
        """
        ...
        if not self._has_minimum_columns(cells, self.config.CLUB_TABLE_MIN_COLUMNS):
            return None
