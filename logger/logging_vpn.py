...
class VpnHandlerLogger:
        ...
        try:
            payload = {
                "streams": [{
                    ...
                    "values": [
                        [str(int(datetime.now(timezone.utc).timestamp() * 1000000000)), 
    ...
    def startup(self, config_dict: Dict[str, Any] = None, **context):
        ...
        """
        self._log_structured("INFO", "component_startup", 
    ...
    def shutdown(self, reason: str = "normal", final_stats: Dict = None, **context):
        ...
        """
        uptime_seconds = (datetime.now(timezone.utc) - self._component_start_time).total_seconds()
        self._log_structured("INFO", "component_shutdown", 
    ...
    def vpn_initialization_started(self, config_use_vpn: bool, **context):
        ...
        """
        self._log_structured("INFO", "vpn_initialization_started", 
                           "VPN manager initialization started",
                           use_vpn=config_use_vpn,
                           **context)
        
    def vpn_initialization_completed(self, vpn_enabled: bool, duration_seconds: float, **context):
        """
        Log successful VPN initialization
        """
        self._log_structured("INFO", "vpn_initialization_completed", 
                           "VPN manager initialized successfully",
                           vpn_enabled=vpn_enabled,
                           duration_seconds=duration_seconds,
                           **context)
        
    def vpn_initialization_failed(self, error: str, error_type: str, duration_seconds: float, **context):
        """
        Log VPN initialization failure
        """
        level = "CRITICAL" if error_type == "VpnRequiredError" else "ERROR"
        self._log_structured(level, "vpn_initialization_failed", 
    ...
    def vpn_rotation_started(self, trigger_reason: str, current_ip: str, request_count: int, **context):
        """
        Log VPN rotation initiation
        """
        self._log_structured("INFO", "vpn_rotation_started", 
    ...
    def vpn_rotation_completed(self, duration_seconds: float, new_ip: str, old_ip: str, **context):
        """
        Log successful VPN rotation
        """
        self._log_structured("INFO", "vpn_rotation_completed", 
    ...
    def vpn_rotation_failed(self, error: str, duration_seconds: float, current_ip: str, **context):
        """
        Log VPN rotation failure
        """
        self._log_structured("ERROR", "vpn_rotation_failed", 
                           "VPN rotation failed",
                           error=str(error),
                           duration_seconds=duration_seconds,
                           current_ip=current_ip,
    ...
    def tunnelblick_recovery_attempted(self, failure_reason: str, **context):
        ...
        """
        self._log_structured("WARNING", "tunnelblick_recovery_attempted", 
    ...
    def tunnelblick_recovery_succeeded(self, duration_seconds: float, **context):
        ...
        """
        self._log_structured("INFO", "tunnelblick_recovery_succeeded", 
    ...
    def tunnelblick_recovery_failed(self, error: str, **context):
        ...
        """
        self._log_structured("CRITICAL", "tunnelblick_recovery_failed", 
                           "Tunnelblick recovery failed - manual intervention required",
                           error=str(error),
                           **context)
    
    # === SECURITY & IP MANAGEMENT ===
    def ip_quota_exceeded(self, current_ip: str, request_count: int, max_requests: int, **context):
        """Log IP request quota exceeded"""
        self._log_structured("WARNING", "ip_quota_exceeded", 
    ...
    def security_violation_detected(self, violation_type: str, current_ip: str, request_count: int, **context):
        """Log security violation detection"""
        self._log_structured("ERROR", "security_violation_detected", 
    ...
    def ip_security_alert_sent(self, alert_message: str, current_ip: str, request_count: int, **context):
        """Log critical security alert"""
        self._log_structured("CRITICAL", "ip_security_alert_sent", 
    ...
    def reboot_required_triggered(self, reason: str, rotation_failures: int = None, **context):
        """Log system reboot requirement"""
        self._log_structured("CRITICAL", "reboot_required_triggered", 
    ...
    def request_timing_started(self, operation: str, vpn_protection_active: bool, **context):
        """Log request timing initiation"""
        self._log_structured("INFO", "request_timing_started", 
                           "Request timing check started",
                           operation=operation,
                           vpn_protection_active=vpn_protection_active,
                           **context)
        
    def request_delay_applied(self, operation: str, delay_seconds: float, delay_type: str, **context):
        """Log request delay application"""
        level = "WARNING" if delay_seconds > 10 else "INFO"
        self._log_structured(level, "request_delay_applied", 
    ...
    def vpn_protection_violation(self, required: bool, active: bool, **context):
        ...
        self._log_structured("ERROR", "vpn_protection_violation", 
    ...
    def vpn_termination_detected(self, error_message: str, **context):
        ...
        self._log_structured("CRITICAL", "vpn_termination_detected", 
    ...
    def vpn_cleanup_started(self, reason: str, **context):
        ...
        self._log_structured("INFO", "vpn_cleanup_started", 
    ...
    def vpn_cleanup_completed(self, duration_seconds: float, **context):
        ...
        self._log_structured("INFO", "vpn_cleanup_completed", 
    ...
    def vpn_cleanup_failed(self, error: str, **context):
        ...
        self._log_structured("WARNING", "vpn_cleanup_failed", 
                           "VPN resource cleanup failed",
                           error=str(error),
                           **context)
    
    # === PERFORMANCE & HEALTH MONITORING ===
    def performance_metric(self, metric_name: str, value: float, threshold: float = None, **context):
        """Log performance metrics with threshold checking"""
        level = "WARNING" if threshold and value > threshold else "INFO"
        self._log_structured(level, "performance_metric", 
    ...
    def rotation_success_rate_metric(self, success_rate: float, total_rotations: int, **context):
        """Log VPN rotation success rate"""
        level = "ERROR" if success_rate < 0.8 else "WARNING" if success_rate < 0.95 else "INFO"
        self._log_structured(level, "rotation_success_rate_metric", 
                           "VPN rotation success rate metric",
                           success_rate=success_rate,
                           total_rotations=total_rotations,
                           **context)
    
    def ip_time_threshold_warning(self, current_ip: str, time_on_ip_seconds: float, threshold_seconds: float, **context):
        """Log warning when IP has been used too long"""
        self._log_structured("WARNING", "ip_time_threshold_warning", 
    ...
    def health_check(self, status: str, is_active: bool, vpn_protection_active: bool, 
                    monitoring_active: bool, details: Dict = None, **context):
        """Log component health status"""
        level = "ERROR" if status.lower() in ["failed", "unhealthy"] else "INFO"
        self._last_health_check = datetime.now(timezone.utc)
        
        self._log_structured(level, "health_check", 
    ...
    def statistics_collected(self, stats_type: str, statistics: Dict, **context):
        ...
        self._log_structured("INFO", "statistics_collected", 
                           f"Statistics collected: {stats_type}",
                           stats_type=stats_type,
                           statistics=statistics,
                           **context)
    
    def rotation_history_recorded(self, rotation_count: int, recent_rotations: List[Dict] = None, **context):
        """Log rotation history update"""
        self._log_structured("INFO", "rotation_history_recorded", 
    ...
    def retry_attempt(self, operation: str, attempt: int, max_attempts: int, error: str = None, **context):
        """Log retry attempts for any operation"""
        self._log_structured("WARNING", "retry_attempt", 
    ...
    def recovery_succeeded(self, operation: str, attempts: int, total_duration_seconds: float, **context):
        """Log successful recovery after retries"""
        self._log_structured("INFO", "recovery_succeeded", 
    ...
    def recovery_failed(self, operation: str, final_error: str, total_attempts: int, **context):
        """Log failed recovery - usually critical"""
        self._log_structured("CRITICAL", "recovery_failed", 
    ...
    def configuration_changed(self, config_section: str, old_value: Any, new_value: Any, **context):
        """Log configuration changes"""
        self._log_structured("INFO", "configuration_changed", 
    ...
    def environment_validation_failed(self, validation_type: str, error: str, **context):
        """Log environment validation failures"""
        self._log_structured("ERROR", "environment_validation_failed", 
    ...
    def get_logger_stats(self) -> Dict[str, Any]:
        ...
        """
        uptime = (datetime.now(timezone.utc) - self._component_start_time).total_seconds()
        return {
